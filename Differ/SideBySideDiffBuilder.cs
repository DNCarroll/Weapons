...
namespace Differ
{
    public class SideBySideDiffBuilder : ISideBySideDiffBuilder
    {
        ...
        private SideBySideDiffModel BuildLineDiff(string oldText, string newText)
        {
            ...
            var diffResult = differ.CreateLineDiffs(oldText, newText, true, true);
        ...
        private static void BuildDiffPieces(DiffResult diffResult, List<DiffPiece> oldPieces, List<DiffPiece> newPieces, PieceBuilder subPieceBuilder)
        {
            ...
            foreach (var diffBlock in diffResult.DiffBlocks)
            {
                ...
                {
                    var oldPiece = new DiffPiece(diffResult.PiecesOld[i + diffBlock.DeleteStartA], ChangeType.Modified, aPos + 1);
                    var newPiece = new DiffPiece(diffResult.PiecesNew[i + diffBlock.InsertStartB], ChangeType.Modified, bPos + 1);
                ...
                if (diffBlock.DeleteCountA > diffBlock.InsertCountB)
                {
                    ...
                    {
                        oldPieces.Add(new DiffPiece(diffResult.PiecesOld[i + diffBlock.DeleteStartA], ChangeType.Modified, aPos + 1));
                        newPieces.Add(new DiffPiece(null, ChangeType.Modified, aPos + 1));
                        aPos++;
                    }
                }
                else
                {
                    ...
                    {
                        newPieces.Add(new DiffPiece(diffResult.PiecesNew[i + diffBlock.InsertStartB], ChangeType.Modified, bPos + 1));
                        oldPieces.Add(new DiffPiece(null, ChangeType.Modified, bPos + 1));
                        bPos++;
                    }
                }
            }
